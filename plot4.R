...
par(mfrow=c(2,2))
plot(epowdata$datetime, as.numeric(epowdata$Global_active_power), type="s", xlab="", ylab="Global Active Power")
plot(epowdata$datetime, as.numeric(epowdata$Voltage), type="s", xlab="datetime", ylab="Voltage")
...
legend("topright", legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"), col=c("black","red","blue"), bty="n", lty=1, xjust=0.5, cex=0.95)
plot(epowdata$datetime, as.numeric(epowdata$Global_reactive_power), type="s", xlab="datetime", ylab="Global_reactive_power")
dev.off()
